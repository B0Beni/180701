...
from telegram import ReplyKeyboardMarkup, ResidentialAddress, ReplyKeyboardRemove
...
markup = ReplyKeyboardMarkup(reply_buttos, one_time_keyboard=False)
...
async def start(update, context):
    ...
    await update.message.reply_html(
        rf'Привет {user.mention_html()} !  Я эхо-бот. Напишите мне что-либо...', reply_markup=markup)


async def bot(update, context):
    await update.message.reply_text('Я пока учусь, как завещал великий Ленин!', reply_markup=markup)

async def help_command(update, context):
    await update.message.reply_text('Я просто даю справки', reply_markup=markup)

async def address(update, context):
    await update.message.reply_text('Вэвэвэ Ленинград эспэбэ точка ру', reply_markup=markup)

async def site(update, context):
    await update.message.reply_text('http://vk.com/xromosomy', reply_markup=markup)
